...
function Toggle({
    ...
    return (
        <button
            ...
            disabled={disabled}
            className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus-visible:outline-none disabled:opacity-50 disabled:cursor-not-allowed ${checked ? 'bg-linkedin' : 'bg-slate-200'
                }`}
        ...
        <div className="flex gap-1 p-1 bg-slate-100 rounded-[8px]">
            {options.map((opt) => (
                <button
                    ...
                    className={`relative flex-1 py-1.5 px-3 text-sm font-medium rounded-[6px] transition-all duration-150 ${value === opt.value
...
function VoiceProfileSection() {
    ...
    const handleSaveTopics = async () => {
        ...
        try {
            ...
            toast.error(err.response?.data?.error || 'Could not save topics.');
        } finally {
            setIsSaving(false);
        }
    };
...
export default function SettingsPage() {
    const router = useRouter();
    const { data: user, isLoading, mutate } = useSWR<UserData>('/api/auth/me', fetcher);
