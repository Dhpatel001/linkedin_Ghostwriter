...
function BillingPage() {
    ...
    const { data: user, isLoading: userLoading } = useSWR<UserData>('/api/auth/me', fetcher);
    ...
    const handleSelectPlan = async (tier: Tier) => {
        ...
        try {
            ...
            toast.error(err.response?.data?.error || 'Could not initiate checkout. Try again.');
            setLoadingTier(null);
        }
    };
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {PLANS.map((plan, i) => (
                            ...
                            >
                                <PlanCard
                                    plan={plan}
                                    isCurrentPlan={currentTier === plan.tier && isSubscribed}
